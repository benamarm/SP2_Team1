...
public class AddBoekenController {
	...
	private void handleDoorgaan() {
		...
		if (selectedBooks.size() > 0 & selectedOpleiding != null) {
			...
			if (boeken.getSelectionModel().getSelectedItems().size() > 1) {
				lWarning.setText(boeken.getSelectionModel().getSelectedItems().size()
						+ " boeken werden toegevoegd aan -" + selectedOpleiding.getNaam() + "-");

			} else {
				lWarning.setText("\"" + boeken.getSelectionModel().getSelectedItem().getIsbn()
						+ "\" werd toegevoegd aan -" + selectedOpleiding.getNaam() + "-");
			}
	...
	private void setBoeken() {
		...
		ObservableList<Boek> boekenInOpleiding = FXCollections
				.observableArrayList(OpleidingDAO.getBoeken(selectedOpleiding));
		GoogleBooksExecutableQuery query = new GoogleBooksExecutableQuery(GoogleBooksQueryPrefix.TITEL,
				selectedOpleiding.getNaam());
